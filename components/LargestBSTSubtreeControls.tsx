...
const LargestBSTSubtreeControls = ({
  numNodes,
  onNumNodesChange,
  speed,
  onSpeedChange,
    ...
    <div className="bg-slate-800 p-4 rounded-lg shadow-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        <div className="space-y-2">
          <label htmlFor="num-nodes-largest-bst" className="block text-sm font-medium text-slate-300">
            Número de Nós: <span className="font-bold text-cyan-400">{numNodes}</span>
          </label>
          <input
            id="num-nodes-largest-bst"
            type="range"
            min="1"
            max="25"
            value={numNodes}
        ...
        <div className="space-y-2">
          <label htmlFor="speed-largest-bst" className="block text-sm font-medium text-slate-300">
            Velocidade da Animação
          </label>
          <input
            id="speed-largest-bst"
            type="range"
            min="10"
            max="1000"
            value={speed}
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="text-center sm:text-right bg-slate-900/50 p-3 rounded-lg flex-grow">
            <p className="text-lg font-semibold text-slate-200">
                Tamanho da Maior BST: <span className="font-mono text-emerald-400 text-2xl">{result ?? '-'}</span>
            </p>
        </div>
      </div>
    </div>
...
export default LargestBSTSubtreeControls;
