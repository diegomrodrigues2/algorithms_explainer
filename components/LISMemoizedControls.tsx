...
interface ControlsProps {
    ...
    onPlayPause: () => void;
    onReset: () => void;
    lisLength: number | null;
}

const LISMemoizedControls = ({
  sequence,
  onSequenceChange,
  speed,
  onSpeedChange,
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="text-center sm:text-right bg-slate-900/50 p-3 rounded-lg flex-grow">
             <p className="text-lg font-semibold text-slate-200">
                Comprimento da LIS: <span className="font-mono text-emerald-400 text-2xl">{lisLength ?? '-'}</span>
            </p>
        </div>
      </div>
    </div>
...
export default LISMemoizedControls;
