...
const SubsetSumMemoizedControls = ({
  numbers,
  onNumbersChange,
  target,
  onTargetChange,
    ...
    <div className="bg-slate-800 p-4 rounded-lg shadow-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        <div className="space-y-2">
          <label htmlFor="numbers" className="block text-sm font-medium text-slate-300">
          ...
          <input
            ...
            placeholder="ex: 5, 10, 12, 13, 15, 18"
            disabled={isPlaying}
          />
        </div>
        ...
        <div className="space-y-2">
           <label htmlFor="target" className="block text-sm font-medium text-slate-300">
            Soma Alvo (máx. 30 para visualização)
          </label>
          <input
            id="target"
            type="number"
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="text-center sm:text-right bg-slate-900/50 p-3 rounded-lg flex-1">
            {result !== null ? (
                 <p className="text-lg font-semibold text-slate-200">
                    Solução Existe? <span className={`font-mono text-2xl ${result ? 'text-emerald-400' : 'text-red-400'}`}>{result ? 'Sim' : 'Não'}</span>
                </p>
            ) : (
...
export default SubsetSumMemoizedControls;
