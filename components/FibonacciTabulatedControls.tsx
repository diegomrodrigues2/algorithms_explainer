...
const FibonacciTabulatedControls = ({
  n,
  onNChange,
  speed,
  onSpeedChange,
    ...
    <div className="bg-slate-800 p-4 rounded-lg shadow-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        <div className="space-y-2">
          <label htmlFor="n-input" className="block text-sm font-medium text-slate-300">
            Valor de n: <span className="font-bold text-cyan-400">{n}</span>
          </label>
          <input
            id="n-input"
            type="range"
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="text-center sm:text-right bg-slate-900/50 p-3 rounded-lg flex-grow">
             <p className="text-lg font-semibold text-slate-200">
                Resultado: <span className="font-mono text-emerald-400 text-2xl">{result ?? '-'}</span>
            </p>
        </div>
      </div>
    </div>
...
export default FibonacciTabulatedControls;