...
import type { MVCTreeAlgorithmStep, MVCTreeNode, MVCTreeEdge, MVCTreeHighlightType } from '../types';
...
const generateRandomTree = (numVertices: number): { nodes: MVCTreeNode[], edges: MVCTreeEdge[], adj: number[][] } => {
    const nodes: MVCTreeNode[] = Array.from({ length: numVertices }, (_, id) => ({ id, x: 0, y: 0 }));
    const edges: MVCTreeEdge[] = [];
...
const layoutTree = (nodes: MVCTreeNode[], adj: number[][], rootId: number): MVCTreeNode[] => {
    const levels: number[][] = [];
...
export const useMVCTree = (initialNodes: number) => {
    const [numNodes, setNumNodes] = useState(initialNodes);
    const [graph, setGraph] = useState<{ nodes: MVCTreeNode[], edges: MVCTreeEdge[] }>({ nodes: [], edges: [] });
    const [steps, setSteps] = useState<MVCTreeAlgorithmStep[]>([]);
    ...
    const generateSteps = useCallback((numVertices: number) => {
        ...
        const newSteps: MVCTreeAlgorithmStep[] = [];
        const take: { [nodeId: number]: number | null } = {};
        const skip: { [nodeId: number]: number | null } = {};
        
        const addStep = (message: string, highlights: { [nodeId: number]: MVCTreeHighlightType } = {}, result: number | null = null) => {
        ...
        addStep("Iniciando o cálculo da Cobertura de Vértices Mínima (MVC).", {});
        if (numVertices > 0) {
            dfs(rootId, -1);
            
            const mvcSize = Math.min(take[rootId]!, skip[rootId]!);
            addStep(`Cálculo completo. Tamanho da MVC é ${mvcSize}. Reconstruindo o conjunto...`, {}, mvcSize);

            // Reconstruct the set for final highlighting
            const mvcSet = new Set<number>();
            const q: [number, number, boolean][] = [[rootId, -1, take[rootId]! <= skip[rootId]!]];
            const highlights: { [nodeId: number]: MVCTreeHighlightType } = {};
            ...
            while(q.length > 0) {
                ...
                if(shouldTake) {
                    ...
                    for(const v of adj[u]) {
                        if (v !== p) q.push([v, u, true]); // Must take children
                    }
                }
            }
            addStep(`Cobertura de Vértices Mínima reconstruída. Tamanho: ${mvcSize}.`, highlights, mvcSize);
        } else {
             addStep(`Árvore vazia. Tamanho da MVC é 0.`, {}, 0);
        }
