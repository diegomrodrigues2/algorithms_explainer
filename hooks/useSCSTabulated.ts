...
import type { SCSTabulatedAlgorithmStep } from '../types';

const INITIAL_SPEED = 400;
const MIN_SPEED = 10;
const MAX_SPEED = 1000;

export const useSCSTabulated = (initialStringA: string, initialStringB: string) => {
    const [stringAInput, setStringAInput] = useState(initialStringA);
    const [stringBInput, setStringBInput] = useState(initialStringB);
    const [steps, setSteps] = useState<SCSTabulatedAlgorithmStep[]>([]);
    ...
    const generateSteps = useCallback((stringA: string, stringB: string) => {
        ...
        const newSteps: SCSTabulatedAlgorithmStep[] = [];
        ...
        addStep("Casos base: SCS com uma string vazia é o comprimento da outra.", {});
        ...
        for (let i = 1; i <= m; i++) {
            for (let j = 1; j <= n; j++) {
                ...
                if (stringA[i - 1] === stringB[j - 1]) {
                    dpTable[i][j] = 1 + (dpTable[i - 1][j - 1] ?? 0);
                    addStep(`Caracteres iguais. dp[${i}][${j}] = 1 + dp[${i-1}][${j-1}] = ${dpTable[i][j]}.`, { ...highlights, matchCell: [i - 1, j - 1]});
                } else {
                    const up = dpTable[i - 1][j] ?? 0;
                    const left = dpTable[i][j - 1] ?? 0;
                    dpTable[i][j] = 1 + Math.min(up, left);
                    addStep(`Caracteres diferentes. dp[${i}][${j}] = 1 + min(dp[${i-1}][${j}], dp[${i}][${j-1}]) = 1 + min(${up}, ${left}) = ${dpTable[i][j]}.`, { ...highlights, upCell: [i - 1, j], leftCell: [i, j - 1]});
                }
            }
        }
        
        addStep("Cálculo da tabela DP concluído. Reconstruindo a SCS...");
        ...
        while (i > 0 && j > 0) {
            ...
            if (stringA[i - 1] === stringB[j - 1]) {
                ...
                i--;
                j--;
            } else if ((dpTable[i - 1][j] ?? Infinity) <= (dpTable[i][j - 1] ?? Infinity)) {
                scs = stringA[i-1] + scs;
                i--;
            } else {
                scs = stringB[j-1] + scs;
        ...
        const finalResult = { length: dpTable[m][n] as number, supersequence: scs };
        addStep(`Finalizado. SCS: "${scs}", Comprimento: ${finalResult.length}.`, { path }, finalResult);
